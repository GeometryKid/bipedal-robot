...
int x_flag=0;
int y_flag=0;
vector_t mytarget = vector_t::Zero(6);
TargetTrajectories cmdVelToTargetTrajectories(const vector_t& cmdVel, const SystemObservation& observation)
{
 //这里传进来的cmdVel是在TargetTrajectorie.h里文件滤波过后的
  const vector_t currentPose = observation.state.segment<6>(6);
  vector_t VelOffset(3);
  if(REAL_OR_SIM == 0)
  {//仿真
   // std::cerr<<"sim"<<std::endl;
    VelOffset<< 0.0,0.0,0.0;
   // VelOffset<< 0.15,-0.15,0;
  }
  else 
  {//实机
   // std::cerr<<"real"<<std::endl;
    VelOffset<< 0.0,0.0,0.0;
    //VelOffset<< 0.05,-0.1,0;
  }
  const Eigen::Matrix<scalar_t, 3, 1> zyx = currentPose.tail(3);
  if(yaw_flag==0)
  {
      yaw_flag=1;
      yaw_set=currentPose(3);
      mytarget(3)=currentPose(3);
  }
  //std::cerr<<"yaw_set: "<<yaw_set<<std::endl;
  //std::cerr<<"currentPose: "<<currentPose.transpose()<<std::endl;

  //std::cerr<<"mytarget: "<<mytarget.transpose()<<std::endl;
  vector_t cmdVelRot = getRotationMatrixFromZyxEulerAngles(zyx) * (cmdVel.head(3) - VelOffset.head(3));
 // std::cerr<<"cmdVelRot: "<<cmdVelRot.transpose()<<std::endl;
 ...
 //  cmdVelRot(0) = 0;
  ...
  //  cmdVelRot(1) = 0;
  //std::cout<<" cmdVelRot "<< cmdVelRot(0) <<std::endl;
  //std::cout<<"test: " << cmdVelRot(0) * timeToTarget <<std::endl;
  const vector_t targetPose = [&]() {
    vector_t target(6);
    target(0) =  currentPose(0)+ cmdVelRot(0) * 0.2; //0.2
    //target(0) =  currentPose(0);
   // target(1) = currentPose(1);
    target(1) = currentPose(1)+ cmdVelRot(1) * 0.4; //0.4
    target(2) = COM_HEIGHT;
    target(3) = currentPose(3) + cmdVel(3) * timeToTarget;
    /*
    if (fabs(cmdVel(3))<0.001&&turn_flag==1) 
    {
      turn_flag=0;
      yaw_set=currentPose(3);
    }
    if (fabs(cmdVel(3))>=0.001) 
    {
      turn_flag=1;
    }
    if(turn_flag==0)
 ...
 /* const vector_t targetPose = [&]() {
    vector_t target(6);
   // std::cerr<<"cmdVel(3): "<<cmdVel(3)<<std::endl;
    
    if (fabs(cmdVelRot(0))<0.001&&x_flag==1) 
    {
      x_flag=0;
      x_set=currentPose(0);
    }
    if (fabs(cmdVelRot(0))>=0.001) 
    {
      x_flag=1;
    }

    if (fabs(cmdVelRot(1))<0.001&&y_flag==1) 
    {
      y_flag=0;
      y_set=currentPose(1);
    }
    if (fabs(cmdVelRot(1))>=0.001) 
    {
      y_flag=1;
    }
 ...
 // trajectories.stateTrajectory[1].head(3) << cmdVelRot;
  trajectories.stateTrajectory[0].head(3) << 0, 0, 0;
  trajectories.stateTrajectory[1].head(3) << 0,0, 0;
  return trajectories;
}
