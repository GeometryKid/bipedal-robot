...
{
  //得到 pinocchioInterfacePtr_ 和CentroidalModelInfo_
  std::cerr << "[LeggedInterface] Settin68886..." << std::endl;
  setupModel(taskFile, urdfFile, referenceFile, verbose);

  initialState_.setZero(centroidalModelInfo_.stateDim);
  loadData::loadEigenMatrix(taskFile, "initialState", initialState_);

  //得到referenceManagerPtr_ = std::make_shared<SwitchedModelReferenceManager>
  //（swingTrajectoryPlanner = std::make_unique<SwingTrajectoryPlanner>）
...
/******************************************************************************************************/
// Set up pre-computation中文注释：设置预计算
/******************************************************************************************************/
/******************************************************************************************************/
...
/******************************************************************************************************/
// Initialize input cost weight中文注释：初始化输入成本权重
/******************************************************************************************************/
/******************************************************************************************************/
                                                                     ...
                                                                     const CentroidalModelInfo& info, bool verbose)
{
 ...
 // {
    ...
    std::cerr << "\n #### =============================================================================\n";
    std::cerr <<"info.stateDim: "<<info.stateDim<<std::endl;
    std::cerr <<"info.inputDim: "<<info.inputDim<<std::endl;
    std::cerr << "Q:\n" << Q << "\n";
    std::cerr << "Q:\n" << Q << "\n";
    std::cerr << "R:\n" << R << "\n";
    std::cerr << " #### =============================================================================\n";
...
{
  std::cerr << "info.actuatedDofNum: " << info.actuatedDofNum << std::endl;
  const size_t totalContactDim = 3 * info.numThreeDofContacts + 6 * info.numSixDofContacts;
  ...
  for (int leg = 0; leg < info.numThreeDofContacts; leg++)
  {
    state_input_limit_penalty[info.actuatedDofNum + info.actuatedDofNum + leg] = std::make_unique<DoubleSidedPenalty>(
        0, 150, std::make_unique<RelaxedBarrierPenalty>(force_limit_barrier_penalty_config));
  }
...
{
  std::cerr << "modelSettings_.positionErrorGain844: " << modelSettings_.positionErrorGain << std::endl;
  auto eeZeroVelConConfig = [](scalar_t positionErrorGain) {
    EndEffectorLinearConstraint::Config config;
    config.b.setZero(3);
    config.Av.setIdentity(3, 3);

    //config.Av(0, 0) = 20;  // 设置第一个元素为 5
   // config.Av(1, 1) = 20;  // 设置第二个元素为 5
