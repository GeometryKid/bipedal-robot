...
bool LeggedController::init(hardware_interface::RobotHW* robot_hw, ros::NodeHandle& controller_nh)
{
  ...
  controller_nh.param("lift_assist/enabled", liftAssistEnabled_, true);
  controller_nh.param<std::string>("lift_assist/topics/left", liftTopics_[0], "/lift/left/command");
  controller_nh.param<std::string>("lift_assist/topics/center", liftTopics_[1], "/lift/center/command");
  controller_nh.param<std::string>("lift_assist/topics/right", liftTopics_[2], "/lift/right/command");
  controller_nh.param("lift_assist/left/force_scale", liftForceScale_[0], 0.30);
  controller_nh.param("lift_assist/center/force_scale", liftForceScale_[1], 0.30);
  controller_nh.param("lift_assist/right/force_scale", liftForceScale_[2], 0.30);
  controller_nh.param("lift_assist/left/force_bias", liftForceBias_[0], 0.0);
  controller_nh.param("lift_assist/center/force_bias", liftForceBias_[1], 0.0);
  controller_nh.param("lift_assist/right/force_bias", liftForceBias_[2], 0.0);
  controller_nh.param("lift_assist/left/max_force", liftForceMax_[0], 320.0);
  controller_nh.param("lift_assist/center/max_force", liftForceMax_[1], 320.0);
  controller_nh.param("lift_assist/right/max_force", liftForceMax_[2], 320.0);

  liftCmdPubs_[0] = controller_nh.advertise<std_msgs::Float64>(liftTopics_[0], 1);
  liftCmdPubs_[1] = controller_nh.advertise<std_msgs::Float64>(liftTopics_[1], 1);
  liftCmdPubs_[2] = controller_nh.advertise<std_msgs::Float64>(liftTopics_[2], 1);
  liftAssistDebugPub_ = controller_nh.advertise<std_msgs::Float64MultiArray>("lift_assist_force", 10);
  //////////////////////////////////////////////////////////////////////////////////
...
//wbc_force_pub_.publish(wbc_force_msg);
  ...
  publishLiftAssistCommand(optimizedState, optimizedInput, loadControllerFlag_ && setWalkFlag_ && !emergencyStopFlag_);
}

void LeggedController::publishLiftAssistCommand(const vector_t& optimizedState, const vector_t& optimizedInput,
                                                bool controllerEnabled)
{
  (void)optimizedState;
  std::array<double, 3> liftCmd{ 0.0, 0.0, 0.0 };

  if (liftAssistEnabled_ && controllerEnabled && optimizedInput.size() >= 12)
  {
    const double leftSupport = std::max(0.0, static_cast<double>(optimizedInput(2))) +
                               std::max(0.0, static_cast<double>(optimizedInput(8)));
    const double rightSupport = std::max(0.0, static_cast<double>(optimizedInput(5))) +
                                std::max(0.0, static_cast<double>(optimizedInput(11)));
    const double centerSupport = 0.5 * (leftSupport + rightSupport);

    liftCmd[0] = liftForceScale_[0] * leftSupport + liftForceBias_[0];
    liftCmd[1] = liftForceScale_[1] * centerSupport + liftForceBias_[1];
    liftCmd[2] = liftForceScale_[2] * rightSupport + liftForceBias_[2];

    for (size_t i = 0; i < liftCmd.size(); ++i)
    {
      liftCmd[i] = std::max(0.0, std::min(liftCmd[i], liftForceMax_[i]));
    }
  }
  ...
  for (size_t i = 0; i < liftCmd.size(); ++i)
  {
    debugMsg.data[i] = liftCmd[i];
  }
  liftAssistDebugPub_.publish(debugMsg);
}
