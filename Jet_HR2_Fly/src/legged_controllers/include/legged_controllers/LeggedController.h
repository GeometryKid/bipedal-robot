...
protected:
  ...
  void ModeSubscribe();
  void publishLiftAssistCommand(const vector_t& optimizedState, const vector_t& optimizedInput, bool controllerEnabled);
...
private:
  ...
  std::atomic<scalar_t> kp_hip_pitch_stance{ 0 };
  std::atomic<scalar_t> kp_hip_pitch_swing{ 0 };
  std::atomic<scalar_t> kd_hip_pitch_stance{ 0 };
  std::atomic<scalar_t> kd_hip_pitch_swing{ 0 };

  std::atomic<scalar_t> kp_hip_roll_stance{ 0 };
  std::atomic<scalar_t> kp_hip_roll_swing{ 0 };
  std::atomic<scalar_t> kd_hip_roll{ 0 };

  std::atomic<scalar_t> kp_hip_yaw_stance{ 0 };
  std::atomic<scalar_t> kp_hip_yaw_swing{ 0 };
  std::atomic<scalar_t> kd_hip_yaw{ 0 };

  std::atomic<scalar_t> kp_knee_stance{ 0 };
  std::atomic<scalar_t> kp_knee_swing{ 0 };
  std::atomic<scalar_t> kd_knee{ 0 };
  ...
  ros::Publisher cmd_actau_pub_;
  
  ros::Publisher mpc_force_pub_;
  ros::Publisher wbc_force_pub_;

  std::array<ros::Publisher, 3> liftCmdPubs_;
  ros::Publisher liftAssistDebugPub_;
  std::array<std::string, 3> liftTopics_{ { "/lift/left/command", "/lift/center/command", "/lift/right/command" } };
  std::array<double, 3> liftForceBias_{ { 0.0, 0.0, 0.0 } };
  std::array<double, 3> liftForceScale_{ { 0.30, 0.30, 0.30 } };
  std::array<double, 3> liftForceMax_{ { 320.0, 320.0, 320.0 } };
  bool liftAssistEnabled_{ true };
};
