...
//ros::Rate ra(100000); 
  ...
  {  
      ...
      check=Check_Sum(RECEIVE_DATA_SIZE-1,READ_DATA_CHECK);  //BCC check passes or two packets are interlaced //BCC校验通过或者两组数据包交错
      ...
      if(error == 0)
      {
        ...
        for(int i=0;i<10;i++)
        {
          dm4340_fbdata(motors[i],&Receive_Data.rx[1+i*5]);
