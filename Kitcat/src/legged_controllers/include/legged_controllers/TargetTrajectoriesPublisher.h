...
public:
    ...
    , tf2_(buffer_)
  {
    ...
    changeLimit_ << 0.02, 0.05, 0.04, 0.3;
    lastVel_ << 0, 0, 0, 0;

    // cmd_vel subscriber
    //下面有定义 cmdVelSub_ = nh.subscribe<geometry_msgs::Twist>("/cmd_vel", 1, cmdVelCallback);
    auto cmdVelCallback = [this](const geometry_msgs::Twist::ConstPtr& msg) {
       //std::cout << "cmdVelCallback766"<< " " << msg->linear.x << " " << msg->linear.y << " " << msg->angular.z<< " " 
